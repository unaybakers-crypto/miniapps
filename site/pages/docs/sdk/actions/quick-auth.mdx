...
Store this token in memory (i.e. React state) and include it as an authorization
header to make authenticated requests to your server.
...
To optimize performance, provide a `preconnect` hint to the browser so that
it can preemptively initiate a connection with the Quick Auth Server:
